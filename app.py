...
if analyze_btn and query:
    ...
    with st.spinner("CrewAI is analyzing your request..."):
        try:
            result_raw = run_crew(final_query)
    ...
    # Read the JSON output generated by the agent
    df = None
    stats = {}
    ...
    if result_raw:
        try:
            import re
            # Extract JSON if the model wrapped it in markdown code blocks by accident
            json_str = result_raw
            ...
            pass # We will handle fallback below
            
    # FALLBACK: If the code execution agent failed to return JSON, we parse the parameters from the query agent and fetch directly
    if df is None or df.empty:
        try:
            import re
            import yfinance as yf
            
            # Find the Stock Data Analyst's JSON output in the logs
            log_text = captured.full_log
            # Look for the JSON block following "Stock Data Analyst"
            match = re.search(r'{\s*"symbols":\s*\[(.*?)\].*?}', log_text, re.DOTALL)
            if match:
                parsed_params = json.loads(match.group(0))
                symbol = parsed_params['symbols'][0]
                
                # Convert natural language timeframe to yfinance timeframe (e.g. "1 year" -> "1y")
                tf_raw = str(parsed_params.get('timeframe', '1y')).lower()
                timeframe = '1y'
                if 'mo' in tf_raw: timeframe = ''.join(filter(str.isdigit, tf_raw)) + 'mo'
                elif 'y' in tf_raw: timeframe = ''.join(filter(str.isdigit, tf_raw)) + 'y'
                elif 'd' in tf_raw: timeframe = ''.join(filter(str.isdigit, tf_raw)) + 'd'
                if not timeframe or timeframe == 'mo' or timeframe == 'y': timeframe = '1y'
                ...
                if not data.empty:
                    df = data.reset_index()
                    stats = {
                        'current_price': round(data['Close'].iloc[-1].item() if isinstance(data['Close'].iloc[-1], pd.Series) else data['Close'].iloc[-1], 2),
                        '%_change': round(((data['Close'].iloc[-1] - data['Close'].iloc[0]) / data['Close'].iloc[0]).item() * 100 if isinstance(data['Close'].iloc[-1], pd.Series) else ((data['Close'].iloc[-1] - data['Close'].iloc[0]) / data['Close'].iloc[0]) * 100, 2),
                        'mean': round(data['Close'].mean().item() if isinstance(data['Close'].mean(), pd.Series) else data['Close'].mean(), 2),
                        'std_dev': round(data['Close'].std().item() if isinstance(data['Close'].std(), pd.Series) else data['Close'].std(), 2)
                    }
        except Exception as e:
            st.error(f"Agent failed to return data and fallback fetch also failed: {e}")
    ...
    if df is not None and not df.empty:
        try:
            ...
            if 'Close' in df.columns:
                # Handle multi-level columns if yfinance returned them
                ...
                fig = px.line(df, x=date_col, y=close_col, title="Stock Price")
            ...
            st.write("Raw output was:")
            st.code(result_raw)
    else:
        st.warning("The agents completed the analysis but didn't return any usable data. Please try rephrasing your query.")
